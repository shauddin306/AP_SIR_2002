...
def auto_correct_and_queue(extracted_voters, assembly_no, part_no, page_num):
    ...
    for ocr_v in extracted_voters:
        ...
        if needs_name_update or needs_rel_update:
            print(f"Mismatch Found! DB: '{db_name}' / '{db_rel}' | OCR: '{ocr_name}' / '{ocr_rel}' (Scores: Name={name_score}%, Rel={rel_score}%)")
            
            queue_data = {
                'voter_id': db_v['id'],
                'source_pdf': ocr_v['source_pdf'],
                'page_no': ocr_v['page_no'],
                'db_voter_name_english': db_v.get('voter_name_english'),
                'db_voter_name_telugu': db_name,
                'db_relative_name_english': db_v.get('relative_name_english'),
                'db_relative_name_telugu': db_rel,
                'ocr_voter_name_english': None,
                'ocr_voter_name_telugu': ocr_name if ocr_name else None,
                'ocr_relative_name_english': None,
                'ocr_relative_name_telugu': ocr_rel if ocr_rel else None,
                'db_epic_id': db_v.get('epic_id'),
                'db_house_no': db_v.get('house_no'),
                # Set OCR EPIC and House equal to DB to avoid EPIC/House diff in UI
                'ocr_epic_id': db_v.get('epic_id'),
                'ocr_house_no': db_v.get('house_no'),
                'similarity_score': round((name_score + rel_score) / 2, 2) if (db_name and ocr_name and db_rel and ocr_rel) else (name_score if db_name and ocr_name else rel_score),
                'status': 'PENDING'
            }
            
            queue_res = supabase.table('voter_staging_queue').insert(queue_data).execute()
            if queue_res.data:
                print(f" -> Pushed serial {db_v['serial_no']} to Staging Queue successfully.")
            else:
                print(f" -> Failed to push serial {db_v['serial_no']} to staging queue.")
